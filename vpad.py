...
font_select=tk.StringVar()
font_box=ttk.Combobox(tool_bar,textvariable=font_select,width=30,state='readonly')
font_box['values']=font_family
font_box.current(font_family.index('Arial'))
font_box.grid(row=0,column=0,padx=5)

#font size
size_select=tk.IntVar()
font_size=ttk.Combobox(tool_bar,textvariable=size_select,width=20,state='readonly')
...
def find_file(event=None):
    ...
    find_dialogue.geometry("400x200+500+300")
    ...
    #entryframe
    find_entry=tk.Entry(find_frame,width=20)
    find_entry.grid(row=0,column=1,padx=4,pady=4)
    replace_entry=tk.Entry(find_frame,width=20)
...
# toolbar and statusbar functionality
var_toolbar=tk.BooleanVar()
var_toolbar.set(True)
var_statusbar=tk.BooleanVar()
...
view.add_checkbutton(label='Tool Bar',image=tool_bar_icon,onvalue=True,offvalue=0,variable=var_toolbar,compound=tk.LEFT,command=toolbar)
view.add_checkbutton(label='Status Bar',image=status_bar_icon,onvalue=1,offvalue=False,variable=var_statusbar,compound=tk.LEFT,command=statusbar)
...
def change_theme():
    ...
    text_editor.config(fg=fgcolor,bg=bgcolor)
